...
const getFollowers = async (req, res) => {
  ...
  const followersData = await Follow.find({ toUserId: user_id })
    .sort({ createdAt: -1 })
    .limit(amount)
    .skip((page - 1) * amount);

  let total = await Follow.find({ toUserId: user_id }).count();
  total = Math.ceil(total / amount);

  const UsersData = await Users.find(
    {
      _id: {
        $in: followersData.map((follow) => follow.fromUserId),
      },
    },
  ...
  const followers = followersData.map((follow) => {
    const currentUser = UsersData.find((user) => user._id == follow.fromUserId);
